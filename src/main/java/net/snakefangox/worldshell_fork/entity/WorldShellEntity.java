...
import net.snakefangox.worldshell_fork.storage.*;
import net.snakefangox.worldshell_fork.util.WSNbtHelper;
    ...
    protected ActionResult handleInteraction(PlayerEntity player, Hand hand, boolean interact) {
        ...
        if (rayCastResult.getType() == HitResult.Type.BLOCK) {
            if (interact) {
                ...
                BlockState blockState = microcosm.getBlockState(rayCastResult.getBlockPos());
                ActionResult result;
                if (blockState.getBlock() instanceof ShellAwareBlock awareBlock) {
                    result = awareBlock.onUseWorldshell(this, blockState, microcosm.getProxyWorld(), rayCastResult.getBlockPos(), player, hand, rayCastResult);
                } else {
                    result = blockState.onUse(microcosm.getProxyWorld(), player, hand, rayCastResult);
                }
                if (result.isAccepted()) {
                    return result;
                }
                World playerWorld = player.world;
                player.world = microcosm.getProxyWorld();
                result = player.getStackInHand(hand).useOnBlock(new ItemUsageContext(player, hand, rayCastResult));
